...
| Local public gate | Checked on 2026-06-29 with `PYTHON_BIN=.venv/bin/python bash scripts/verify_public.sh` -> `52 passed`, `public verification passed` |
...
4. Read the committed reviewer acceptance evidence in `docs/evidence/reviewer-acceptance-report.txt`.
5. Read [Demo Walkthrough](./DEMO_WALKTHROUGH.md).
6. Read [Technical Review Packet](./TECHNICAL_REVIEW_PACKET.md).
7. Read [Reviewer Evidence Pack](./REVIEWER_EVIDENCE_PACK.md).
8. Read [Employer Trigger Proof](./EMPLOYER_TRIGGER_PROOF.md).
9. Read [First Slice Playbook](./FIRST_SLICE_PLAYBOOK.md).
10. Read [Live Owner Proof](./LIVE_OWNER_PROOF.md).
11. Read [Production Readiness Drill](./PRODUCTION_READINESS_DRILL.md).
12. Read [Credentialed Sandbox Preflight](./CREDENTIALED_SANDBOX_PREFLIGHT.md).
13. Inspect the latest live combined sandbox run: https://github.com/AlexGerlitz/ai-ops-workflow-kit/actions/runs/27799329429
14. Inspect `docs/evidence/live-telegram-approval.txt`, `docs/evidence/bitrix24-contract.txt`, and `docs/evidence/bitrix24-sandbox-preflight.txt`.
15. Inspect `rag_quality` in `python3 scripts/run_offer_demo.py` output or call `POST /rag/eval` after importing the demo playbook.
16. Inspect `GET /reviewer/observability` for the read-only runtime/counter/RAG/privacy/outbox snapshot.
17. Inspect `docs/evidence/live-postgres-persistence.txt` for the API-restart persistence proof.
18. If `https://saleops.duckdns.org/` is reachable, run `python3 scripts/reviewer_acceptance_report.py`.
19. If the alias is needed, run `bash scripts/smoke_live_demo.sh https://leadscore.duckdns.org`.
