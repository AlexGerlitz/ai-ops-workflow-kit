...
| Business scenario replay | `docs/evidence/business-scenario-replay.txt` compresses the offer demo into business input, backend route, RAG quality, approval state, CRM handoff, and Bitrix24 dry-run evidence |
...
| Employer trigger evidence | `docs/EMPLOYER_TRIGGER_PROOF.md` maps AI workflow/RAG, CRM/API integration, backend/platform ownership, and DevOps reliability triggers to concrete repo evidence and first-result evidence |
| First slice playbook | `docs/FIRST_SLICE_PLAYBOOK.md` maps role/project context to RAG/transcript, CRM handoff, human approval, and reliability slices with done criteria and verification commands |
...
| RAG quality evidence | `/demo/run` and `POST /rag/eval` return `rag_quality.ok=true`, `passed=2/2`, expected source matches, required terms, score floor, and citations |
...
| Live Bitrix24 read-only sandbox | `docs/evidence/bitrix24-sandbox-preflight.txt` -> `profile=passed`, `crm_lead_fields=passed`, `origin=<redacted-bitrix24-origin>` |
...
9. Read [Employer Trigger Evidence](./EMPLOYER_TRIGGER_PROOF.md).
10. Read [First Slice Playbook](./FIRST_SLICE_PLAYBOOK.md).
11. Read [Live Owner Evidence](./LIVE_OWNER_PROOF.md).
...
18. Inspect `docs/evidence/live-postgres-persistence.txt` for the API-restart persistence evidence.
